            ...
            async function sendMessage() {
                ...
                if (rawText) {
                    try {
                        ...
                        // 3. SAFE AUDIT LOG ENCRYPTION (Prevents DataError Crashes)
                        if (typeof HOS_PUBLIC_KEY !== 'undefined' && HOS_PUBLIC_KEY.length > 50) {
                            adminCiphertext = await encryptMessage(rawText, HOS_PUBLIC_KEY);
                        } else {
                            console.warn("Audit Log Skipped: Master Padlock not found or invalid on server.");
                        }
                ...
                // Attach Admin's copy safely!
                if (adminCiphertext && adminCiphertext !== "") {
                        ...
                        if (data.success) {
                            ...
                            if (editIdElement && editIdElement.value !== "") {
                                ...
                                if (msgRow) {
                                    ...
                                    if (textBody) textBody.innerHTML = rawText.replace(/\n/g, '<br>');
